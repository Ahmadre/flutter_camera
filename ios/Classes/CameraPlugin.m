...
- (void)captureOutput:(AVCapturePhotoOutput *)output
  ...
  bool success = [UIImageJPEGRepresentation(image, 1.0) writeToFile:_path
                                                         atomically:YES];
  if (!success) {
    _result([FlutterError errorWithCode:@"IOError"
...
- (UIImageOrientation)getImageRotation {
  ...
  BOOL (^isNearValue)(float value1, float value2) =
      ^BOOL(float value1, float value2) {
        return fabsf(value1 - value2) < threshold;
      };
  BOOL (^isNearValueABS)(float value1, float value2) =
      ^BOOL(float value1, float value2) {
  ...
  // If none of the above, then the device is likely facing straight down or
  // straight up -- just pick something arbitrary
...
@interface FLTCam
    : NSObject <FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate,
                AVCaptureAudioDataOutputSampleBufferDelegate,
                FlutterStreamHandler>
@property(readonly, nonatomic) int64_t textureId;
@property(nonatomic, copy) void (^onFrameAvailable)();
@property BOOL enableAudio;
@property BOOL autoInitializeFlash;
@property BOOL autoInitializeAutoExposure;
...
- (instancetype)initWithCameraName:(NSString *)cameraName
               ...
               autoInitializeFlash:(BOOL)autoInitializeFlash
        autoInitializeAutoExposure:(BOOL)autoInitializeAutoExposure
...
- (instancetype)initWithCameraName:(NSString *)cameraName
  ...
  _captureVideoInput =
      [AVCaptureDeviceInput deviceInputWithDevice:_captureDevice
                                            error:&localError];
  if (localError) {
    *error = localError;
    return nil;
  ...
  [_captureVideoOutput setSampleBufferDelegate:self
  ...
  if (autoInitializeFlash) {
    [self setFlashMode:autoInitializeFlash];
  }

  if (autoInitializeAutoExposure) {
    [self setAutoExposureMode:autoInitializeAutoExposure];
  }
...
- (void)setCaptureSessionPreset:(ResolutionPreset)resolutionPreset {
  ...
  case max:
    if ([_captureSession canSetSessionPreset:AVCaptureSessionPresetHigh]) {
      ...
      _previewSize = CGSizeMake(
          _captureDevice.activeFormat.highResolutionStillImageDimensions.width,
          _captureDevice.activeFormat.highResolutionStillImageDimensions
              .height);
      break;
    }
  case ultraHigh:
    ...
    if ([_captureSession canSetSessionPreset:AVCaptureSessionPresetLow]) {
      ...
      NSError *error = [NSError
          errorWithDomain:NSCocoaErrorDomain
                     code:NSURLErrorUnknown
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"No capture session available "
                                               @"for current capture session."
                 }];
      @throw error;
    }
  }
  ...
  if (_isStreamingImages) {
    if (_imageStreamHandler.eventSink) {
      ...
      for (int i = 0; i < planeCount; i++) {
        ...
        NSData *bytes = [NSData dataWithBytes:planeAddress
                                       length:length.unsignedIntegerValue];
        ...
        planeBuffer[@"bytes"] =
            [FlutterStandardTypedData typedDataWithBytes:bytes];
  ...
  if (_isRecording && !_isRecordingPaused) {
    if (_videoWriter.status == AVAssetWriterStatusFailed) {
      _eventSink(@{
        @"event" : @"error",
        @"errorDescription" :
            [NSString stringWithFormat:@"%@", _videoWriter.error]
      });
      return;
    }
    ...
    if (output == _captureVideoOutput) {
      if (_videoIsDisconnected) {
        ...
        if (_videoTimeOffset.value == 0) {
          _videoTimeOffset =
              CMTimeSubtract(currentSampleTime, _lastVideoSampleTime);
        } else {
          CMTime offset =
              CMTimeSubtract(currentSampleTime, _lastVideoSampleTime);
      ...
      CMTime nextSampleTime =
          CMTimeSubtract(_lastVideoSampleTime, _videoTimeOffset);
      [_videoAdaptor appendPixelBuffer:nextBuffer
      ...
      if (_audioIsDisconnected) {
        ...
        if (_audioTimeOffset.value == 0) {
          _audioTimeOffset =
              CMTimeSubtract(currentSampleTime, _lastAudioSampleTime);
        } else {
          CMTime offset =
              CMTimeSubtract(currentSampleTime, _lastAudioSampleTime);
    ...
    if (![_videoWriterInput appendSampleBuffer:sampleBuffer]) {
      _eventSink(@{
        ...
        @"errorDescription" : [NSString
            stringWithFormat:@"%@", @"Unable to write to video input"]
      });
    }
  }
}
  ...
  if (_audioWriterInput.readyForMoreMediaData) {
    if (![_audioWriterInput appendSampleBuffer:sampleBuffer]) {
      _eventSink(@{
        @"event" : @"error",
        @"errorDescription" : [NSString
            stringWithFormat:@"%@", @"Unable to write to audio input"]
      });
    }
  }
}
...
- (void)startVideoRecordingAtPath:(NSString *)path
  ...
  if (!_isRecording) {
    if (![self setupWriterForPath:path]) {
      _eventSink(
          @{@"event" : @"error", @"errorDescription" : @"Setup Writer Failed"});
      return;
    }
    _isRecording = YES;
    ...
    _eventSink(@{
      @"event" : @"error",
      @"errorDescription" : @"Video is already recording!"
    });
  }
}
  ...
  if (_isRecording) {
    ...
    NSError *error =
        [NSError errorWithDomain:NSCocoaErrorDomain
                        ...
                        userInfo:@{
                          NSLocalizedDescriptionKey : @"Video is not recording!"
                        }];
...
- (BOOL)setupWriterForPath:(NSString *)path {
  ...
  NSDictionary *videoSettings = [NSDictionary
      dictionaryWithObjectsAndKeys:AVVideoCodecH264, AVVideoCodecKey,
                                   [NSNumber numberWithInt:_previewSize.height],
                                   AVVideoWidthKey,
                                   [NSNumber numberWithInt:_previewSize.width],
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  CameraPlugin *instance =
      [[CameraPlugin alloc] initWithRegistry:[registrar textures]
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if (_dispatchQueue == nil) {
    _dispatchQueue =
        dispatch_queue_create("io.flutter.camera.dispatchqueue", NULL);
  }
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call
  ...
  if ([@"availableCameras" isEqualToString:call.method]) {
    ...
    NSNumber *autoInitializeFlash = call.arguments[@"autoInitializeFlash"];
    NSNumber *autoInitializeAutoExposure =
        call.arguments[@"autoInitializeAutoExposure"];
    NSError *error;
    FLTCam *cam = [[FLTCam alloc]
                initWithCameraName:cameraName
                  resolutionPreset:resolutionPreset
                       enableAudio:[enableAudio boolValue]
               autoInitializeFlash:[autoInitializeFlash boolValue]
        autoInitializeAutoExposure:[autoInitializeAutoExposure boolValue]
