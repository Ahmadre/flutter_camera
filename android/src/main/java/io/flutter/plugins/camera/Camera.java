...
public class Camera {
  private static final String TAG = "Camera[Camera2]";
  ...
  private final boolean autoInitializeFlash;
  private final boolean autoInitializeAutoExposure;
  ...
  public Camera(final Activity activity, final FlutterView flutterView, final String cameraName,
      final String resolutionPreset, final boolean enableAudio, final boolean autoInitializeFlash,
      final boolean autoInitializeAutoExposure) throws CameraAccessException {
    if (activity == null) {
    ...
    this.autoInitializeFlash = autoInitializeFlash;
    this.autoInitializeAutoExposure = autoInitializeAutoExposure;
  ...
  private void createCaptureSession(int templateType, Runnable onSuccessCallback, Surface... surfaces)
    ...
    previewBuilder = cameraDevice.createCaptureRequest(templateType);

    // Build Flutter surface to render to
    SurfaceTexture surfaceTexture = flutterTexture.surfaceTexture();
    surfaceTexture.setDefaultBufferSize(previewSize.getWidth(), previewSize.getHeight());
    Surface flutterSurface = new Surface(surfaceTexture);
    previewBuilder.addTarget(flutterSurface);
    ...
    //     autoInitializeFlash ? CaptureRequest.FLASH_MODE_TORCH : CaptureRequest.FLASH_MODE_OFF);

    // // Request initial Auto Exposure mode
    // previewBuilder.set(CaptureRequest.CONTROL_AE_MODE,
    //     autoInitializeAutoExposure ? CaptureRequest.CONTROL_AE_MODE_ON : CaptureRequest.CONTROL_AE_MODE_OFF);
